...
def onnx_taint_shape(*combined_args, start=None, end=None):
  """The impl for https://github.com/onnx/onnx/blob/v1.12.0/docs/Operators.md#Shape."""

  input_args, taint_input_args = taint_handler.split_args(combined_args, op="shape")

  assert len(input_args) == 1
  assert len(taint_input_args) == 1

  result = shape.onnx_shape(*input_args, start=start, end=end)
  result = taint_propagation.identity_like(result)

  return result


class TaintShape(shape.Shape, taint_handler.TaintHandler):

  @classmethod
  def jit_lookup(cls):
    return {shape.onnx_shape: onnx_taint_shape}
