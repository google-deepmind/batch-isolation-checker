...
from batching_security_checker.onnx_ops import layernormalization as taint_ln
...
class TaintSimplifiedLayerNormalization(
    simplifiedlayernormalization.SimplifiedLayerNormalization,
    taint_handler.TaintHandler,
):
  """Taint handler for SimplifiedLayerNormalization operator."""

  @classmethod
  def jit_lookup(cls):
    return {
        layernormalization.onnx_layernormalization: (
            taint_ln.onnx_taint_layernormalization
        )
    }

  @classmethod
  def data_handler(cls):
    return simplifiedlayernormalization.SimplifiedLayerNormalization
